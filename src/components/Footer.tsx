...
export default function Footer() {
  const socials = [
    { url: labInfo.githubUrl, label: 'GitHub', icon: <Github size={18} /> },
    { url: labInfo.youtubeUrl, label: 'YouTube', icon: <Youtube size={18} /> },
    { url: labInfo.bilibiliUrl, label: 'Bilibili', icon: <BilibiliIcon size={18} /> },
    { url: labInfo.scholarUrl, label: 'Google Scholar', icon: <GraduationCap size={18} /> },
          ...
          <div className="flex items-center justify-center gap-3">
            {socials.map((s) => (
              <a
                ...
                aria-label={s.label}
                title={s.label}
                className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-50 border border-gray-200 text-gray-500 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-colors"
              >
                {s.icon}
              </a>
            ))}
